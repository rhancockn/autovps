...
class Siemens(object):
	...
	def __init__(self, path):
		...
		scalar_fields = ['MagneticFieldStrength', 'ImagingFrequency', 
			'MixingTime', 'EchoTime', 'RepetitionTime', 'ImaCoilString',
		 	...
		 	'VoiInPlaneRotation', 'DataPointColumns', 'RealDwellTime', 'PixelBandwidth',
		 	'ImagedNucleus']

		for k in scalar_fields:
			self.meta[k] = csareader.get_scalar(self.csa, k)

		self.meta['ImagePositionPatient'] = csareader.get_vector(self.csa, 'ImagePositionPatient',3)
		...
		self.meta['ImageOrientationPatient'] = csareader.get_vector(self.csa, 'ImageOrientationPatient', 6)
	...
	def read_data(self, conj=True):
		...
		if os.path.isfile(self.path):
			...
			#the directory must contain more than one file, as this is checked in the 
			#class constructor
			...
			#this assumes different interleaved acquisitions have the same (0020, 0012) Acquisition Number 
			...
			#the first two instances of eja sequences are single
			#TODO: figure out what they are
			#in the meantime, assume the first match the last two, which are missing the same channel
			n_reps = lastinstance-2

			#TODO: handle channel combined data
			if len(files) != (n_reps*(n_channels +1)):
				raise Exception('Expected n_reps[%d] * (n_channels[%d]+1 files' % (n_reps, n_channels))
		...
		#take the complex conjugate, which is what Tarquin seems to expect			
		if conj:
			data = np.conj(data)
