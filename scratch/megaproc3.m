%-- 9/17/18, 2:00 PM --%
cd('/Volumes/RHANCOCK_K/20180830_143255_S4688BYM1')
scan=1
out_spectra=cell(1,3);
off_spectra=cell(1,3);
on_spectra=cell(1,3);
ccGiven=false;
water=[];
load(['/Volumes/RHANCOCK_K/20180830_143255_S4688BYM1/MEGA'  num2str(scan) '.mat']);
raw=svs;
filestring=['/Volumes/RHANCOCK_K/20180830_143255_S4688BYM1/mega' num2str(scan)];
cd(filestring)
...
help op_alignMPSubspecs
out_ph
help op_alignAverages
out_aa
...
off_spectra{scan}=subSpec1;
on_spectra{scan}=subSpec2;
...
writ='y';
if writ=='y' || writ=='Y'
RF=io_writelcm(diffSpec,['./_diff_lcm'],diffSpec.te);
RF=io_writelcm(subSpec1,['./_editON_lcm'],subSpec1.te);
RF=io_writelcm(subSpec2,['./_editOFF_lcm'],subSpec2.te);
if water
RF=io_writelcm(outw,['./_w_lcm'],outw.te);
end
end
close all;
...
fclose(fid);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
out_spectra{scan}=out;
end
on_ave=on_spectra{1};
off_ave=off_spectra{1};
for i=2:length(on_spectra)
on_ave=op_concatAverages(on_ave, on_spectra{i});
off_ave=op_concatAverages(off_ave, off_spectra{i});
end
on_ave = op_averaging(on_ave);
off_ave = op_averaging(off_ave);
ave = op_concatSubspecs(on_ave, off_ave);
diffSpec=op_combinesubspecs(ave,'summ');
...
aligned=op_alignAllScans(out_spectra);
%Make fully processed data;
diffSpec=op_combinesubspecs(out,'diff');
sumSpec=op_combinesubspecs(out,'summ');
subSpec1=op_takesubspec(out,1);
...
ave
help op_alignAverages
[out_aa,fs,phs]=op_alignAverages(ave,.25,'y');
out_aa
out=op_alignMPSubspecs(out_aa);
out_av=op_averaging(out_aa);
out=op_alignMPSubspecs(out_av);
out
plot(out.ppm, real(out.specs(:,1)))
hold on
plot(out.ppm, real(out.specs(:,2)), 'r')
plot(out.ppm, real(out.specs(:,2)+out.specs(:,1)), 'k')
plot(out.ppm, real(-out.specs(:,2)+out.specs(:,1)), 'k')
close all
phs
out_ave
out_av
plot(real(out_av.specs(:,1)))
plot(real(out_av.specs(:,2)))
help op_alignMPSubspecs
out=op_alignMPSubspecs(out_av);
plot(out.ppm, real(out.specs(:,1)))
plot(out.ppm, real(out.specs(:,2)))
ave = op_concatSubspecs(off_ave, on_ave);
[out_aa,fs,phs]=op_alignAverages(ave,.25,'y');
out_av=op_averaging(out_aa);
out=op_alignMPSubspecs(out_av);
plot(out.ppm, real(out.specs(:,1)))
...
plot(out.ppm, real(out.specs(:,1)))
plot(out.ppm, real(out.specs(:,2)+out.specs(:,1)), 'k')
diffSpec=op_combinesubspecs(out,'summ');
plot(real(diffSpec.specs))
diffSpec=op_combinesubspecs(out,'diff');
plot(real(diffSpec.specs))
RF=io_writelcm(diffSpec,['./ave_diff_lcm'],diffSpec.te);
clear
...
out_spectra{scan}=out_rm;
on_spectra{scan}=op_takesubspec(out_rm,1);
off_spectra{scan}=op_takesubspec(out_rm,2);
end
on_comb=on_spectra{1};
off_comb=off_spectra{1};
for i=2:length(on_spectra)
on_comb=op_concatAverages(on_comb, on_spectra{i});
off_comb=op_concatAverages(off_comb, off_spectra{i});
end
combined = op_concatSubspecs(on_comb, off_comb);
combined
out_rm = op_concatSubspecs(on_comb, off_comb);
%NOW ALIGN AVERAGES:  A.K.A. Frequency Drift Correction.
driftCorr='y';
if driftCorr=='n' || driftCorr=='N'
out_av=op_averaging(out_rm);
